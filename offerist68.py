...
def edit_str():
    global data, be_txt
    a = int(tree.selection()[0])
    if be_txt == 'Edit string':
        be_txt = 'Save changes'
        entry1.delete(0, END)
        entry1.insert(0, data[a][0])
        entry1a.delete(0, END)
        entry1a.insert(0, data[a][1])
        entry1b.delete(0, END)
        entry1b.insert(0, data[a][2])
        entry1c.delete(0, END)
        entry1c.insert(0, data[a][3])
        button4e.configure(text = be_txt)
        return
    else:
        data[a][0] = str(entry1.get())
        data[a][1] = str(entry1a.get())
        data[a][2] = str(entry1b.get()).replace(',', '.')
        data[a][3] = str(entry1c.get())
        trv()
        be_txt = 'Edit string'
        button4e.configure(text = be_txt)
        entry1.delete(0, END)
...
def pdf_out():
    ...
    for j in range(len(td)):
        if j%8 == 0:
            data1.append(cepure)
        if td[j] != cepure and not td[j] in data1:
            data1.append(td[j])
    
    pdf = FPDF()
    pdf.add_font('Hasklig-Light', '', 'Font/LiberationSans-Regular.ttf', uni=True)
    pdf.add_font('Hasklig-Bold', '', 'Font/LiberationSans-Bold.ttf', uni=True)
...
button2 = Button(f0,width=15, text="Formula", command = myformula)
...
button2 = Button(f_1,width=15, text="+", command = plus)
button2.pack(side=LEFT, expand=1, padx = 10)

labelf1 = Label(ff, width=12, text = 'Code')
labelf1.pack(side=LEFT, pady=0)
labelf1a = Label(ff, width=40, text = 'Description')
...
button1 = Button(f_3,width=15, text="PDF", command = pdf_out)
button1.pack(side=RIGHT, expand=1, padx = 10, pady = 10)
button3 = Button(f_3,width=15, text="Save", command = save_data)
button3.pack(side=RIGHT, expand=1, padx = 10, pady = 10)
button4 = Button(f_3,width=15, text="Load", command=load_data)
button4.pack(side=RIGHT, expand=1, padx = 10, pady = 10)
button5 = Button(f_3,width=15, text="New", command=new_data)
button5.pack(side=RIGHT, expand=1, padx = 10, pady = 10)

button4i0 = Button(f_2,width=15, text= 'INT 1.01=2.00', command = int0)
button4i0.pack(side=RIGHT, expand=1, padx = 10, pady = 10)
button4i1 = Button(f_2,width=15, text= 'INT 1.01=1.00', command = int1)
button4i1.pack(side=RIGHT, expand=1, padx = 10, pady = 10)
button4e = Button(f_2,width=15, text= be_txt, command = edit_str)
button4e.pack(side=RIGHT, expand=1, padx = 10, pady = 10)
button0 = Button(f_2,width=15, text="Sort", command = sort_data)
button0.pack(side=RIGHT, expand=1, padx = 10, pady = 10)
button2d = Button(f_2,width=15, text="DelString", command = delstring)
button2d.pack(side=RIGHT, expand=1, padx = 10, pady = 10)
